...
#define ESC_MIN     1000   // full reverse
#define ESC_NEUTRAL 1500   // stop
#define ESC_MAX     2000   // full forward

#define ESC_ARM_PULSE ESC_NEUTRAL  // pulse to hold during arming window
...
#define MAX_PULSE_STEP 15   // max pulse change allowed per loop iteration (us) -
                             // slows down how fast we can cross the neutral/reverse
                             // boundary, giving the ESC's internal brake/reverse
                             // logic time to settle instead of chattering

#define SPEED_LIMIT_PERCENT 25  // % of full throttle/steering range allowed in 2WD mode
                                 // (100 = full range, no limit)
...
int passThroughMap(int mixedValue) {
  ...
  return map(clamped, STICK_MIN, STICK_MAX, ESC_MIN, ESC_MAX);
}
...
int mirroredMap(int mixedValue) {
  ...
  return map(mirrored, STICK_MIN, STICK_MAX, ESC_MIN, ESC_MAX);
}

// Zeroes out small jitter around center so noise doesn't get amplified through mixing
int applyDeadband(int rawValue, int deadband) {
...
int lastLeftPulse  = ESC_NEUTRAL;
int lastRightPulse = ESC_NEUTRAL;

void setup() {
  Serial.begin(921600);
  IBusSerial.begin(115200, SERIAL_8N1, IBUS_RX_PIN, -1);

  leftESC.attach(LEFT_ESC_PIN, ESC_MIN, ESC_MAX);
  rightESC.attach(RIGHT_ESC_PIN, ESC_MIN, ESC_MAX);

  Serial.println("Arming ESCs - holding neutral throttle...");
  leftESC.writeMicroseconds(ESC_ARM_PULSE);
  rightESC.writeMicroseconds(ESC_ARM_PULSE);
...
void loop() {
  ...
  if (gotFrame) {
    ...
    int steering = applyDeadband(steeringRaw, DEADBAND);
    int throttle = applyDeadband(throttleRaw, DEADBAND);

    // Scale down to the configured percentage of full range (speed cap)
    steering = (steering * SPEED_LIMIT_PERCENT) / 100;
    throttle = (throttle * SPEED_LIMIT_PERCENT) / 100;

    int leftMix  = STICK_NEUTRAL + throttle + steering;
    int rightMix = STICK_NEUTRAL + throttle - steering;

    int leftTarget  = passThroughMap(leftMix);
    int rightTarget = mirroredMap(rightMix);

    lastLeftPulse  = slewLimit(leftTarget, lastLeftPulse, MAX_PULSE_STEP);
    lastRightPulse = slewLimit(rightTarget, lastRightPulse, MAX_PULSE_STEP);

    leftESC.writeMicroseconds(lastLeftPulse);
    rightESC.writeMicroseconds(lastRightPulse);

    Serial.print("Left: ");
    Serial.print(lastLeftPulse);
    Serial.print("  Right: ");
    Serial.println(lastRightPulse);
  }

  // Failsafe: if we haven't seen a valid frame in a while, stop both motors
  if (millis() - lastValidFrameMs > FAILSAFE_TIMEOUT_MS) {
    lastLeftPulse  = ESC_NEUTRAL;
    lastRightPulse = ESC_NEUTRAL;
