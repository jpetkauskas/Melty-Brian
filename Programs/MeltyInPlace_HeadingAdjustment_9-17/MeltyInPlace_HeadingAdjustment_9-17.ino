...
#define MAX_SCALE 6
#define PRINT_INTERVAL 10 // amount of loops (sum time of n loops)
...
void print(const char* string) {
  if (loopTimer > PRINT_INTERVAL) {
    telemetryPrintln(string);
  }
}
void printF(const char* identifier, float value) {
  if (loopTimer > PRINT_INTERVAL) {
    telemetryPrintf(identifier, value);
  }
}
...
void printAccelValues() {
  ...
  printF("Y: %.4f", getAccelY(y));
  printF("Z: %.4f", getAccelZ(z));
}
...
double getHeading(double previousHeading, unsigned long tInitial) { // rotations
  ...
  double rotationalSpeed = sqrt(accelerationY / (ACCELEROMETER_OFFSET * 0.0000284)) / (6 * 1000000) + readDial(); // Rotations per microsecond
...
void meltyDrive(double heading) {
  ...
  double joystickX = (channels[0] - STICK_NEUTRAL)/500.0;
  double joystickY = (channels[1] - STICK_NEUTRAL)/500.0;
...
void setup() {
  ...
  if (!telemetryBegin("Melty Brian", "password")) {
    Serial.println("Wifi failure");
  } else {
    Serial.println("Connected");
    
    telemetryPrintln("Telemetry online");
  }
