...
#define ACCELEROMETER_OFFSET 10 // inches
...
double getHeading(double previousHeading, unsigned long tInitial) { // rotations
  ...
  double accelerationX = analogRead(0); // FIX!!!!!!!!! WONT WORK!!!!
  double accelerationY = analogRead(0);

  double centrifugalAcceleration = hypot(accelerationX, accelerationY); // g
  double rotationalSpeed = sqrt(centrifugalAcceleration / (ACCELEROMETER_OFFSET * 0.0000284)) * (60 * 1000000); // Rotations per microsecond
...
void meltyDrive(double heading) {
  ...
  int throttleConstrainedRight = applyDeadband(throttleRight) * SPEED_LIMIT_MELTY - 29.36605;
  ...
  double baseTransMag = hypot(joystickX, joystickY); // Magnitude of translation vector

  double leftFluctuatingPower = baseTransMag * (500 - throttleConstrainedLeft) * cos(heading - baseTransAng); // The difference in wheel speed as base rotates
